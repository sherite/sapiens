app.factory('GruposFactory', function () {

    var lblGruposIU = '';
    var acciones = '';
    var gruposFactory = {};

    gruposFactory.lblGruposIU = lblGruposIU;
    gruposFactory.acciones = acciones;
    gruposFactory.data = {
        ID: '',
        Name: '',
        Description: '',
...
app.controller('GruposController', function ($scope, $location, $route, $routeParams, $timeout, $compile, $http, $translate, GruposFactory, CommonDataFactory) {

    $scope.$on('languageChanged', function (evt, data) {

        var dTable = $('#GroupsTable').DataTable();
    ...
    $scope.go = function (url, acciones, data) {
        ...
        GruposFactory.lblGruposIU = acciones === 'edit' ? "EDICION DE GRUPOS" : 'ALTA DE GRUPOS';
        GruposFactory.acciones = acciones;
    ...
    $scope.Initialize = function () {
        ...
        $scope.lblGruposIU = $translate.instant('index.titleGroups')
                ...
                columns: [
                    ...
                    },
                    {
                        data: function (row, type, set, meta) {
                            ...
                            var buttonView   = "<a role='button' ng-click='go(\"gruposView\",\"view\"," + JSON.stringify(row) + ")'><span title='" + $translate.instant("index.groupView") + "' class='fa fa-eye'></a>&nbsp&nbsp";
                            var buttonEdit = "<a role='button' ng-click='go(\"gruposEdit\",\"edit\"," + JSON.stringify(row) + ")'><span title='" + $translate.instant("index.groupEdit") + "' class='glyphicon glyphicon-pencil'></span></a>&nbsp&nbsp";
                            var buttonDelete = "<a role='button' ng-click='GruposDelete(" + row.ID + ",\"" + row.Name + "\")'><span title='" + $translate.instant("index.groupDelete") + "' class='glyphicon glyphicon-trash'></span></a>&nbsp&nbsp";
                            var buttonSecurity = "<a role='button' ng-click='go(\"gruposSecurity\",\"view\"," + JSON.stringify(row) + ")'><span title='Permisos de Grupos' class='glyphicon glyphicon-lock'></a>";
    ...
    // BAJA DE GRUPOS
    $scope.GruposDelete = function (ID, Description) {
        var record = ID + (Description || undefined ? " - " + Description : "");

        bAlert($translate.instant("index.groupsDeleteTitle") + Description,
            $translate.instant("index.usersDeleteQuestion"), "",
            bAlertsLevels.DANGER,
            function () {

                $http.delete(urlBase + "/groups/" + ID).then(function (a, b, c) {
...
app.controller('GruposViewController', function ($scope, $location, $route, $routeParams, $timeout, $compile, $translate, $http, GruposFactory) {
    ...
    $scope.Inicializar = function (accion) {
        ...
        if (GruposFactory.acciones === 'view') {

            $scope.lblGruposIU = $translate.instant('index.groupsView')

            $scope.group = {
                ID: GruposFactory.data.ID,
                Name: GruposFactory.data.Name,
                Description: GruposFactory.data.Description,
                Estado: GruposFactory.data.Status,
                Users: GruposFactory.data.Users
            };

            document.getElementById("GRUPO_ESTADO").checked = GruposFactory.data.Status === 1;
        }
        ...
        $http(
            ...
            .then(function (a, b, c) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    for (var j = 0; j < GruposFactory.data.Users.length; j++) {

                        if (GruposFactory.data.Users[j].Name === $scope.myObject[i].Name) {
...
app.controller('GruposEditController', function ($scope, $location, $route, $routeParams, $timeout, $compile, $translate, $http, GruposFactory) {

    $scope.myObject = [];

    $scope.accion = GruposFactory.acciones;
    ...
    $scope.group = {
        ...
        Name: GruposFactory.data.Name,
        Description: GruposFactory.data.Description,
        Status: GruposFactory.data.Status,
        Users: GruposFactory.data.Users,
        Options: GruposFactory.data.Options
    };

    $scope.SaveData = function () {

        $scope.group.Options = $("#lstUsers").val();

        GruposFactory.data.Users = [];

        for (var i = 0; i < $scope.group.Options.length; i++) {
            for (var j = 0; j < $scope.myObject.length; j++) {

                if ($scope.myObject[j].ID.toLocaleString() == $scope.group.Options[i]) {
                    GruposFactory.data.Users.push($scope.myObject[j]);
                }
            }
        }

        $scope.group.Users = GruposFactory.data.Users;

        if ($scope.accion !== 'edit') {

            $scope.lblGruposIU = $translate.instant("index.groupsEdit")

            $scope.group.Status = 1;
            $scope.group.ID = 0;

            GruposFactory.data.ID = $scope.group.ID;
            GruposFactory.data.Name = $scope.group.Name;
            GruposFactory.data.Description = $scope.group.Description;
            GruposFactory.data.Status = $scope.group.Status;
            GruposFactory.data.Users = $scope.group.Users;

            GruposFactory.acciones = "insert";
        }
        else {

            $scope.lblGruposIU = $translate.instant("index.groupsCreate")

            $scope.group.Status = GruposFactory.data.Status;
        }

        $http({
            method: GruposFactory.acciones === 'edit' ? 'PUT' : 'POST',
            url: urlBase + 'groups',
            data: JSON.stringify(GruposFactory.data)
        }).then(function (a, b, c) {
            bAlert(GruposFactory.acciones === 'edit' ?
            ...
            if (GruposFactory.acciones === 'insert') {

                GruposFactory.data = {

                    ID: a.data.Object.ID,
                    Name: a.data.Object.Name,
                    Description: a.data.Object.Description,
            ...
            GruposFactory.acciones = "view";

            $scope.go("gruposView", "view", + GruposFactory.data);
        })
    ...
    $scope.lblGruposIU = GruposFactory.lblGruposIU;

    $scope.checkEstado = function () {

        GruposFactory.data.Status = document.getElementById("GRUPO_ESTADO").checked ? 1 : 2;
    };
    ...
    $scope.Inicializar = function (accion) {
        ...
        if (GruposFactory.acciones === 'edit') {
            ...
            document.getElementById("GRUPO_ESTADO").checked = GruposFactory.data.Status === 1;
            document.getElementById("divGrupo").style.visibility = "visible";
            document.getElementById("divEstado").style.visibility = "visible";
        }
        else {

            document.getElementById("divGrupo").style.visibility = "hidden";
